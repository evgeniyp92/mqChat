...
export class BackendService {
  ...
  async init() {
    // try to read the user from indexedDB
    const data = await this.indexedDBService.getUserId();
    if (data) {
      // User already looked at the page
      const { username } = data;
      if (username) {
      ...
      // Set the username in state to the default anon username
      this.username$.next('Anony Mouse');
      // Purge messages
      await this.indexedDBService.setMessages([]);
      // Set the userid with a default user
      await this.indexedDBService.setUserId('Anony Mouse', false, v4());
    }
    this.isLoading$.next(true);
    const messages = await this.indexedDBService.getMessages();
    this.messages$.next(messages);
    // TODO: Rewrite this to better use piping
    ...
    const intervalSub = interval(1000)
      .pipe(
        // Get the array length in observable form
        switchMap(() =>
          this.messages$.pipe(map((messages) => messages.length)),
        ),
        // Continue only when length is not 0
        filter((length) => length != 0),
        // Take only first occurrence, so it won't continue forever
        first(),
      )
      .subscribe(() => {
        // Set isLoading$ to false
        this.isLoading$.next(false);
        // Unsubscription is not necessary with the 'first' operator,
        // but for best practice - remember to unsubscribe from observables to prevent memory leaks
